-- KytosHub: Sprint 2 - module configuration and cart lending schema
-- 1) carts / cart_loans tables per condominium schema
-- 2) condo_settings defaults (feature flags) for all modules

-- Idempotent helper: provisions Sprint 2 tables + module flag defaults for a tenant schema
...
BEGIN
    ...
    -- Ensure the settings table exists (older provisioned schemas may lack it)
    ...
    -- Only create cart tables when the base structure (departments) exists
    SELECT EXISTS (
    ...
    -- Feature flag defaults per condominium (Sprint 1 modules ON, new modules OFF)
...
-- Backfill existing condominiums
DO $$
DECLARE t record;
BEGIN
